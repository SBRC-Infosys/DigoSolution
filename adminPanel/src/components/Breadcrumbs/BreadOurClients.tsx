...
const BreadOurClients = ({ pageName }: BreadcrumbProps) => {
...
//   const [url,setUrl] = useState<string>("");
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await axiosInstance.post(`/clients/add`,formData , {
        headers: {
          'Content-Type': "multipart/form-data"
        }
      });
      if(res.status === 200 || res.status === 201){
        toast.success(`${companyName} has been successfully added.`);
        queryClient.invalidateQueries({queryKey: ['ourClient']});
        handleOk();
      }
    } catch (error) {
      toast.error(`Failed to add.Try Again!`)
      console.log("Error while adding to our-clients:",error)
    }
  };
...
export default BreadOurClients;
