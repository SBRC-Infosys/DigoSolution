...
const BreadSocial = ({ pageName }: BreadcrumbProps) => {
  ...
  const handleAdd = async () => {
    if(!platformName|| !url || images.length === 0){
    ...
    try {
      const res = await axiosInstance.post(`/social-links/add`,formData , {
        headers: {
          'Content-Type': "multipart/form-data"
        }
      });
      if(res.status === 200 || res.status === 201){
        toast.success(`${platformName} has been successfully added.`);
