...
import { SocialType } from "@/types/socailLinks";
import Link from "next/link";
import Spinner from "../Spinner";
import { useState } from "react";
import { useToast } from "../../../contexts/ToastProvider";
import EditModal from "../Modals/EditSocialLink";
...
const fetchSocialLinks = async () => {
  try {
    ...
    console.error("Error while fetching socail links:", error);
  }
};
...
const TableSix: React.FC = () => {
  ...
  const [item, setItem] = useState<SocialType>();
  ...
  const paginatedData = socialLink?.slice(startIndex, endIndex);

  const handleModalBox = (categoryItem: SocialType) => {
          ...
          {paginatedData?.map((item, key) => (
            ...
            >
              ...
              <div className="flex items-center justify-center py-4 ">
                <Image src={item?.icon} width={60} height={50} alt="Brand" />
              </div>
