...
export const fetchNewsEvents = async () => {
  const response = await axiosInstance.get('/news-events'); 
  return response.data.data;
};

const fetchNewsEventsCategories = async () => {
  const response = await axiosInstance.get('/newseventscategories');
  return response.data.data;
};
...
const TableOne = () => {
  const {
    data: NewsEvents,
    isLoading: NewsEventsLoading,
    error: NewsEventsError,
    refetch
  } = useQuery<NewsAndEvents[]>({
    queryKey: ['newsevents'],
  ...
  const {
    data: NewsEventsCategory,
    isLoading: NewsEventsCategoryLoading,
    error: NewsEventsCategoryError,
  } = useQuery<NewsEventCategory[]>({
    queryKey: ['newsEventsCategory'],
    queryFn: fetchNewsEventsCategories
  });

  const [item, setItem] = useState<NewsAndEvents>();
  ...
  const paginatedContacts = NewsEvents?.slice(startIndex, endIndex);

  // console.log(NewsEvents,NewsEventsCategory)

  const handleModalBox = (Item: NewsAndEvents) => {
    setItem(Item);
    setOpenModal(true);
  };
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await axiosInstance.delete(`/news-events/delete/${id}`);
      ...
      console.error("Error while deleting news-events item:", error);
    }
  };

  if(NewsEventsCategoryLoading || NewsEventsLoading) return <Spinner />
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
            { NewsEvents && NewsEvents.map((packageItem, key) => (
              <tr key={key}>
                ...
                <td className="border-b text-center border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                  <h5 className="font-medium text-black dark:text-white">
                    {NewsEventsCategory?.find(ele=> ele.id === packageItem.category_id)?.name}
                  </h5>
      ...
      {openModal && item && (
        <EditModal
          item={item}
          endPoint="/news-events/update"
          pageName="News & Events"
          isModalOpen={openModal}
          setIsModalOpen={setOpenModal}
          refetch={refetch} 
          endPointForCat={'/newseventscategories'}        />
      )}
    </div>
  );
};
