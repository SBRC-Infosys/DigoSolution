...
import EditModal from '../Modals/ProductEditCat';
import { useToast } from '../../../contexts/ToastProvider';
import axiosInstance from '../../../utils/axiosInstance';

// Define the type for a single news event category
...
async function fetchData() { 
  ...
  try {
    const res = await axiosInstance.get(`/newseventscategories`);
...
const TableThree = () => {
  const [item, setItem] = useState<NewsEventCategory>();
  ...
  const { data, error, isLoading, refetch } = useQuery<NewsEventCategory[]>({
    queryKey: ['newseventscategories'],
    queryFn: fetchData,
  })
  ...
  const handleModalBox = (categoryItem: NewsEventCategory) => {
  ...
  const handleDelete = async (id:number) => {
    try {
      const response = await axiosInstance.delete(`/newseventscategories/delete/${id}`);
      ...
      console.error('Error while deleting product category:',error);
    }
  }
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        ...
        <table className="w-full table-auto">
          <thead  >
            ...
            <th className="w-1/12  px-4 text-center py-4 font-medium text-black dark:text-white xl:pl-11">
              ...
              <th className="w-1/4  px-4 text-center py-4 font-medium text-black dark:text-white xl:pl-11">
                Category Name
              </th>
              <th className="w-1/4  px-4 py-4 text-center font-medium text-black dark:text-white">
                Description
              </th>
              {/* <th className="min-w-[120px] px-4 py-4 font-medium text-black dark:text-white">
                Status
              </th> */}
              <th className="w-1/4 px-4 text-center py-4 font-medium text-black dark:text-white">
                Actions
              </th>
            </tr>
          </thead>
...

      {
        ...
        <EditModal
          category={item}
          endPoint='/newseventscategories/update'
          pageName='News Events Category'
