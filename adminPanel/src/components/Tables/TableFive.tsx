...
import EditModal from "../Modals/EditGallery";
...
const TableFive = () => {
  ...
  const [item, setItem] = useState<GalleryType>();
  ...
  const paginatedData = galleryItems?.slice(startIndex, endIndex);

  const handleModalBox = (categoryItem: GalleryType) => {
        ...
        >
          <div className="col-span-3 flex items-center">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="h-12.5 w-15 rounded-md">
                {product.type === "image" ? (
                  <Image
                    ...
                    loading="eager" // Eager loading to avoid lazy loading warnings
                    layout="intrinsic" // Ensures aspect ratio is preserved
                  />
                ) : getYouTubeVideoId(product.url) ? (
                  <iframe
                    src={`https://www.youtube.com/embed/${getYouTubeVideoId(
                      product.url,
                    )}`}
                    title={`Video of ${product.title}`}
                    width={80}
                    height={60}
                    loading="lazy"
                    allowFullScreen
              ...
              <p className="pl-6 text-sm text-black dark:text-white">
                {product.title}
              </p>
            </div>
          </div>
          ...
          <div className="border-b border-[#eee] py-5 dark:border-strokedark">
            <div className="flex items-start justify-start space-x-3.5">
