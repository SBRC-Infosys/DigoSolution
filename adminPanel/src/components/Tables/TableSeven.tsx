...
import EditModal from "../Modals/EditTestimonial";
...
const TableSeven = () => {
  const {
    ...
    error,
    refetch,
  } = useQuery<TestimonialsType[]>({
    queryKey: ["testimonials"],
    queryFn: fetchTestimonials,
  });

  const [item, setItem] = useState<TestimonialsType>();
  ...
  const paginatedData = testimonialsData?.slice(startIndex, endIndex);

  const handleModalBox = (categoryItem: TestimonialsType) => {
          ...
          <tbody>
            {paginatedData?.map((packageItem, key) => (
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] px-4 py-5 text-center dark:border-strokedark">
                  <p
                    className="line-clamp-3 text-black dark:text-white"
                    dangerouslySetInnerHTML={{ __html: packageItem.content }}
                  ></p>
                </td>
