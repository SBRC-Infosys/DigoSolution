...
// import { Package } from "@/types/testimonials";
...
const BlogCategories = () => {
  ...
  const { data, error, isLoading, refetch } = useQuery<BlogCategory[]>({
    queryKey: ['blogcategory'],
    queryFn: fetchData,
  })

  const [item, setItem] = useState<BlogCategory>();
  ...
  const paginatedContacts = data?.slice(startIndex, endIndex);


  // console.log(data)

  const handleModalBox = (categoryItem: BlogCategory) => {
  ...
  const handleDelete = async (id:number) => {
    try {
      const response = await axiosInstance.delete(`/blogcategory/delete/${id}`);
