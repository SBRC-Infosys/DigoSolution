...
const DropdownUser = () => {
  ...
  const {
    data: userData = [], // Default to an empty array to prevent undefined issues
    isLoading,
    error,
  } = useQuery<UserType[]>({
  ...
  // Logout function
  const handleLogout = () => {
    deleteCookie("user"); // Clear the user cookie
    deleteCookie("token"); // Clear token if applicable
    // Optionally perform additional cleanup or API calls
    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs">{userData?.[0]?.role}</span>
        </span>
