...
const EditModal = ({ item,
  ...
  const [formValues,setFormValues] = useState<GalleryType>(item);
  const [type, setType] = useState<string>(item.type);
  const [images, setImages] = useState<File[]>([]);
  const [localImages, setLocalImages] = useState<ImageType[]>([]);
  const [imgUrls, setImgUrls] = useState<string[]>([type === "image"?item.url:""]);
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await axiosInstance.patch(`/gallery/update/${formValues.id}`,formData , {
        headers: {
          ...
          </div>
          {
            type === "video" ? (
              ...
              <input
                ...
                value={item.type === "video"?item.url:""}
                onChange={handleChange}
                className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
              />
            </div>
