...
const EditModal = ({ item,
  ...
  const [platformName,setPlatformName] = useState<string>(item.platform_name);
  const [url,setUrl] = useState<string>(item.url);
  const [images, setImages] = useState<File[]>([]);
  const [localImages, setLocalImages] = useState<ImageType[]>([]);
  const [imgUrls, setImgUrls] = useState<string[]>([item.icon]);
    ...
    if(!platformName|| !url){
      ...
      try {
        ...
        if(res.status === 200 || res.status === 201){
          toast.success(`${platformName} has been successfully updated.`);
